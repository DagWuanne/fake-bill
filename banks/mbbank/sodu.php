                ...
                if ($_POST['type'] == 'demo' || !empty($username)) {
                        ...
                        $sodu = DB::queryFirstField("SELECT sodu FROM `users` WHERE username = '$username'");
                         if ($sodu >= $tiengoc) {
                             ...
                             DB::insert('notifications', [
                                 'notifications' => 'Đã tạo 1 bill số dư MB, số tiền trừ ' . $tiengoc,
                                 'username' => $username,
                                 'amount' => $tiengoc
                             ]);
                         }
                    }
                    if (empty(trim($_POST['amount'])) || empty(trim($_POST['time_dt']))) {
                    ...
                    if ($sodu >= $tiengoc) {
                        ...
                        if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
                            ...
                            imagedestroy($avatar);
                            $desiredWidth = 122;  // Set desired width
                            $desiredHeight = 120; // Set desired height
                            $circularAvatar = imagescale($circularAvatar, $desiredWidth, $desiredHeight);
                        }
                        // Load the base image
                        $image = imagecreatefrompng($_SERVER['DOCUMENT_ROOT'] . '/banks/mbbank/sodu/sodu-mb.png');
                        ...
                        if (isset($circularAvatar)) {
                            imagecopy($image, $circularAvatar, 105, 347, 0, 0, imagesx($circularAvatar), imagesy($circularAvatar));
                            imagedestroy($circularAvatar);
                        }

                        // Thêm khiên vào avt
                        $khienPath = imagecreatefrompng($_SERVER['DOCUMENT_ROOT'] . '/banks/mbbank/sodu/khien-mb.png');
                        $imgW = imagesx($khienPath) - 17;
                        $imgH = imagesy($khienPath) - 17;

                        $khienPath = imagescale($khienPath, $imgW, $imgH);

                        imagecopy($image, $khienPath, 90, 340, 0, 0, imagesx($khienPath), imagesy($khienPath));
                        imagedestroy($khienPath);
                        imagettftext($image, 30, 0, 130, 75, imagecolorallocate($image, 255, 255, 255), $_SERVER['DOCUMENT_ROOT'] . '/fonts/Inter/Inter-SemiBold.ttf', $_POST['time_dt']);
                        ...
                        $numberX = 330;
                        $numberY = 520;
                        imagettftext($image, $numberSize, 0, $numberX, $numberY, imagecolorallocate($image, 255, 255, 255), $vndFont, number_format($number));

                        $vndBox = imagettfbbox($vndSize, 0, $vndFont, $vndText);
                        $vndWidth = $vndBox[2] - $vndBox[0];
                        $vndX = $numberX + $numberWidth + 10;
                        $vndY = 520;
                        imagettftext($image, $vndSize, 0, $vndX, $vndY, imagecolorallocate($image, 255, 255, 255), $vndFont, $vndText);
                        //imagedestroy($image);
